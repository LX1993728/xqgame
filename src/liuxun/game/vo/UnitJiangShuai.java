...
public class UnitJiangShuai extends Unit {
	public UnitJiangShuai() {
		this.setP(6); // 设置优先级
	}
	public UnitJiangShuai(String name, Person curU, Person otherU) {
		this();
		this.setName(name);
	...
	public UnitJiangShuai(String name, Person curU, Person otherU, int x, int y) {
			...
			if (this.getStatus() == 1 && this.getCurrentU().getFlag() == 0) {
				...
				if (x >= 0 && x <= 7 && toX >= 0 && toX <= 7 && y >= 0 && y <= 3 && toY >= 0 && toY <= 7
					...
					if (xCandadite.contains(toX) && yCandadite.contains(toY) && this.getQp()[toX][toY] != null
							&& this.getQp()[toX][toY].getP() < 6 && this.getQp()[toX][toY].getP() != -1
